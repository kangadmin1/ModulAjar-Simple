...
import { ArrowLeft, Download, RefreshCw, Sparkles, FileText, Printer } from 'lucide-react';
    ...
    <div className="max-w-5xl mx-auto animate-fade-in-up pb-10">
      ...
      <div className="flex flex-col gap-3 mb-6 no-print">
         <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-sm flex flex-wrap justify-between items-center gap-3">
             <button 
              onClick={onReset}
              className="flex items-center gap-2 text-slate-600 hover:text-red-600 transition-colors font-medium px-3 py-2 rounded-lg text-sm mr-auto"
            >
              <ArrowLeft size={18} />
              <span className="hidden md:inline">Kembali</span>
            </button>
            <div className="font-bold text-slate-700 text-sm md:text-base truncate max-w-[150px] md:max-w-xs hidden sm:block">
                {moduleData.title}
            </div>
            ...
            <div className="flex items-center gap-2">
                <button 
                  ...
                  className="flex items-center gap-2 bg-slate-700 text-white px-4 py-2 rounded-lg hover:bg-slate-800 transition-all shadow-md shadow-slate-200 font-bold text-xs md:text-sm active:scale-95"
                >
                  <Printer size={16} />
                  <span className="hidden md:inline">Download PDF (Cetak)</span>
